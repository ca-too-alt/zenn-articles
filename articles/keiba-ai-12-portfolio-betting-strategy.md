...
最終的にどの戦略をポートフォリオに組み込むか？
その判断基準として、単なる回収率ではなく、独自の評価スコア **`betting_score`** を考案しました。

**ファイル**: `memo/馬券購入戦略改修案.txt`

### 計算式

$$ Score = 利益額 - (0.5 \times 最大ドローダウン) - (0.1 \times 平均購入点数) + ボーナス $$

### 各項目の意図

1.  **利益額**: もちろん、稼げる戦略が正義です。
2.  **最大ドローダウンのペナルティ**: 「一時的に大きく負ける」戦略は、運用を続けるのが難しいため、スコアを下げます。
3.  **平均購入点数のペナルティ**: 買い目が多いと、オッズ変動の影響を受けやすく、トリガミのリスクも増えるため、少ない点数で当てる戦略を優遇します。
4.  **的中ボーナス**: まぐれ当たり（数千レースに1回の大穴）だけで評価されないよう、コンスタントに的中する戦略に加点します。

このスコアを使ってOptunaで最適化を行うことで、「利益」と「安定性」のバランスが取れた、実運用に耐えうる戦略を選定しています。

## 4. 最強の布陣（設定ファイル）

選ばれた戦略は、`src/keiba/config/betting_strategies.csv` に保存され、本番システムで読み込まれます。
...
- 独自の `betting_score` を用いて、リスクとリターンのバランスが良い戦略を選定する。
